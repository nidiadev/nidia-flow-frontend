...
const updateWebFormSchema = z.object({
  name: z.string().min(1, 'El nombre es requerido').max(255, 'Máximo 255 caracteres'),
  description: z.string().optional(),
  isActive: z.boolean(),
});

type UpdateWebFormForm = z.infer<typeof updateWebFormSchema>;
...
export default function EditWebFormPage() {
  ...
  const updateWebForm = useMutation({
    mutationFn: (data: any) => webFormsApi.update(formId, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['web-forms'] });
      toast.success('Formulario actualizado exitosamente');
      router.push(route(`/crm/web-forms/${formId}`));
    },
    onError: (error: any) => {
      toast.error(error?.response?.data?.message || 'Error al actualizar el formulario');
    },
  });
  ...
  const {
    ...
    watch,
    reset,
  } = useForm<UpdateWebFormForm>({
    resolver: zodResolver(updateWebFormSchema),
  });
  ...
  const onSubmit = async (data: UpdateWebFormForm) => {
    try {
      await updateWebForm.mutateAsync(data);
    } catch (error) {
    ...
    <ErrorBoundary>
      <div className="space-y-6">
        <SectionHeader
          ...
          description="Modifica el formulario web"
          actions={
            <Button variant="outline" asChild>
              <TenantLink href={route(`/crm/web-forms/${formId}`)}>
                <ArrowLeft className="h-4 w-4 mr-2" />
                Cancelar
              </TenantLink>
            </Button>
                ...
                <CardContent className="space-y-5">
                  <div className="pt-4">
                    <Button 
                      type="submit" 
                      className="w-full" 
                      disabled={isSubmitting || updateWebForm.isPending}
                      size="lg"
                    >
                      {isSubmitting || updateWebForm.isPending ? (
                        <>
